...
sub init { # if script is loaded after connect
	...
	foreach my $server(@servers) {
		if( $server->isupport('NETWORK') eq 'BitlBee' ){
			#print "NEW BITLBEE SERVER = ".$server->{tag};
			$bitlbee_server = $server;
			my @channels = $server->channels();
			foreach my $channel(@channels) {
				if( $channel->{mode} =~ /C/ ){
					#print "NEW BITLBEE CHANNEL = ".$channel->{name};
...
sub privmsg {
	...
	if( $server->{tag} eq $bitlbee_server->{tag} ){
		...
		if( $text =~ /\x02\[\x02\x02\x02.*\x02\]\x02/ ){
			...
			#print "this is a message to display the new timestamp";
			...
			#print "this is a message to display the previous timestamp";
		}
	}
}
